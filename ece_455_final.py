...
def simulate_RM(RM_params):
    ...
    release_queue = [] #initializing queue of tasks at t = 0 based on task priorities, queue contains task #, remaining exec time, and deadline
    for key in priorities:
        preemptions[key] = 0
        release_queue.append([key, executions[key]/TIME_STEP, deadlines[key]/TIME_STEP]) 
    
    released = release_queue.pop(0)
    curr_task = released[0]
    t_exec = released[1]
    curr_deadline = released[2]
    start_time = 0

    for i in range(0, int((hyperperiod/TIME_STEP)+1)): #simulating up until hyperperiod
        if i >= curr_deadline: #ensuring that deadline of curr_task hasn't been missed (>= condition relates to the way I implemented indexing)
            return
        ...
        if (i + 1)/INDEX_TO_TIME not in release_times.keys() and (curr_task == -1 or i < (start_time + t_exec - 1)): #no task(s) released AND (no task currently OR curr task is not finished execution)
            ...
            for task in release_times[(i + 1)/INDEX_TO_TIME]:
                if(curr_task == -1 or priorities[task] < priorities[curr_task] or i >= (start_time + t_exec - 1)): #processor is idle, new task is higher priority than curr task, or curr task JUST finished
                    if curr_task != -1 and i < (start_time + t_exec - 1): #if processor isn't idle and curr_task isn't already finished
                        release_queue.insert(0, [curr_task, executions[curr_task]/TIME_STEP - (i + 1 - start_time), curr_deadline]) #placing curr task, remaining exec time back, and deadlines back on queue
                        preemptions[curr_task] += 1 #incrementing preemption count of curr task

                    curr_task = task #updating curr task
                    t_exec = executions[task]/TIME_STEP #updating remaining exec time
                    start_time = i + 1 #updating start_time
                    curr_deadline = start_time + deadlines[task]/TIME_STEP

                else: #place task in queue according to priority
                    count = 0
                    for n in release_queue:
                        if priorities[task] >= priorities[n[0]]:
                            count += 1
                        else:
                            break
                    release_queue.insert(count, [task, executions[task]/TIME_STEP, i + 1 + deadlines[task]/TIME_STEP])
            ...
            if release_queue:
                ...
                curr_deadline = start_time + released[2]
            else:
